...
def salvar_aparelhos_xml(aparelhos):
    ...
    for aparelho_data in aparelhos:
        ...
        ET.SubElement(aparelho_elem, "id").text = str(aparelho_data["id"])
        ET.SubElement(aparelho_elem, "nome").text = aparelho_data["nome"]
        ET.SubElement(aparelho_elem, "marca").text = aparelho_data["marca"]
        ET.SubElement(aparelho_elem, "preco").text = str(aparelho_data["preco"])
        ET.SubElement(aparelho_elem, "categoria").text = aparelho_data["categoria"]
        ET.SubElement(aparelho_elem, "deposito").text = aparelho_data["deposito"]
    tree = ET.ElementTree(root)
    tree.write(ARQUIVO, encoding="utf-8", xml_declaration=True)
